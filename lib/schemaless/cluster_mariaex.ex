defmodule Schemaless.Cluster.Mariaex do
  use GenServer
  use Calendar
  ...
  def init(args) do
    ...
    to = args[:to]
    step = args[:step]
    IO.puts "Mariaex Connecting to #{host}:#{port} as #{user} #{from}..#{to} step #{step}"
    {:ok, ro_conn} = Mariaex.Connection.start_link(
      username: user <> "_ro",
      password: password,
      port: 3306,
      skip_database: true)
    {:ok, rw_conn} = Mariaex.Connection.start_link(
      username: user <> "_rw",
      password: password,
      port: 3306,
      skip_database: true)
